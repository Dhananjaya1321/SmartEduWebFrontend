...
export const MEoMAdminDashboard = () => {
    const [data, setData] = useState<DashboardData | undefined>(undefined);

    const fetchDashboardData = async () => {
        const response = await dashboardAPIController.getDetailsToMOEDashboard();
        if (response) {
            setData(response)
        }
    }
            ...
            <section className="h-max flex w-[95%] flex-col justify-center ">
                ...
                <section className='text-[#005285] flex flex-row justify-center mt-5 flex-wrap gap-5 '>
                    <div
                        ...
                        <h1 className="text-7xl mb-2">{data?.provinceCount ?? 0}</h1>
                        <h1>Province Count</h1>
                    </div>
                    <div
                        className="shadow-xl rounded w-[300px] h-[200px] bg-white flex flex-col justify-center align-middle">
                        <h1 className="text-7xl mb-2">{data?.zonalCount ?? 0}</h1>
                        <h1>Zonal Count</h1>
                    </div>
                    <div
                        className="shadow-xl rounded w-[300px] h-[200px] bg-white flex flex-col justify-center align-middle">
                        <h1 className="text-7xl mb-2">{data?.schoolsCount ?? 0}</h1>
                        <h1>Schools Count</h1>
                    </div>
                    <div
                        className="shadow-xl rounded w-[300px] h-[200px] bg-white flex flex-col justify-center align-middle">
                    ...
                    <div
                        className="shadow-xl rounded w-[300px] h-[200px] bg-white flex flex-col justify-center align-middle">
                        <h1 className="text-7xl mb-2">{data?.principalsCount ?? 0}</h1>
                        <h1>Principals Count</h1>
                    </div>
                    <div
                        className="shadow-xl rounded w-[300px] h-[200px] bg-white flex flex-col justify-center align-middle">
