import {LoginAside} from "../../../component/LoginAside/LoginAside";
import {FileUploader} from "../../../component/FileUploader/FileUploader";
import {useLocation, useNavigate} from "react-router-dom";
import {useState} from "react";

export const PrincipalVerifyPage = () => {
    const navigate = useNavigate();
    const location = useLocation();
    const personalData = location.state;

    const [nicFront, setNicFront] = useState<string | null>(null);
    const [nicBack, setNicBack] = useState<string | null>(null);
    const [moeFront, setMoeFront] = useState<string | null>(null);
    const [moeBack, setMoeBack] = useState<string | null>(null);
    const [appointment, setAppointment] = useState<string | null>(null);

    const handleNext = () => {
        const combinedData = {
            ...personalData,
            documents: {
                nicFront,
                nicBack,
                moeFront,
                moeBack,
                appointment,
            },
        };

        // Basic validation before proceeding (optional)
        if (!nicFront || !nicBack || !moeFront || !moeBack || !appointment) {
            alert("Please upload all required documents before proceeding.");
            return;
        }

        navigate("/verify-school", {state: combinedData});
    };

    return (
        <section className="relative justify-center items-center flex flex-row w-full h-[700px]">
            <LoginAside btnStatus="Login" height="700px"/>
            <article
                className="loginArticles flex flex-col justify-center sm:w-[40%] h-[700px] bg-white px-8 absolute sm:relative">
                <div className="flex flex-col text-start mb-5">
                    <h1 className="loginH1 text-3xl poppins-semibold">Verify</h1>
                    <p className="loginP">Please verify that you are the principal.</p>
                </div>
                <div className="flex flex-col text-start">
                    <div className="flex items-center justify-center flex-row gap-3">
                        <FileUploader label="NIC front photo" onChange={setNicFront}/>
                        <FileUploader label="NIC back photo" onChange={setNicBack}/>
                    </div>
                    <div className="flex items-center justify-center flex-row gap-3">
                        <FileUploader label="MoE ID card front photo" onChange={setMoeFront}/>
                        <FileUploader label="MoE ID card back photo" onChange={setMoeBack}/>
                    </div>
                    <FileUploader
                        label="Appointment letter pdf or photo"
                        onChange={setAppointment}
                    />
                    <button
                        onClick={handleNext}
                        className="mt-7 bg-[#006CAF] px-6 w-full py-3 rounded-md text-white font-medium"
                    >
                        Next
                    </button>
                </div>
            </article>
        </section>
    );
};
